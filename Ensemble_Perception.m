try
    ...
    % ex: "MS-2"
	if ~isdir([current '/Participant_Data/' nameID]) %if no one has the same initials so far, creates the folder named with just the initials
    	mkdir([current '/Participant_Data/' nameID]);
    else
        s=1;
        name = nameID;
        while isdir([current '/Participant_Data/' nameID]) %while a folder with that name exists, increase the "number" by one
            s=s+1;
            nameID=strcat(name, '-', num2str(s));
            if ~isdir([current '/Participant_Data/' nameID]) %if no folder with that name exists, create the folder and break from the loop
                mkdir([current '/Participant_Data/' nameID]);
                break;
            end
        end
    ...
    for i=1:ntrials
        ...
        if order(i) == 1
            ...
            %for one face, check if it's black or white and set the actual
            %avg to 1 or 10
            if ismember(face, white)
              response(i, 2) = 10;
			elseif ismember(face, black)
              response(i, 2) = 1;
			end
      	end
